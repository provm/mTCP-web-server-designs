...
HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	//printf("[epserver.c HandleReadEvent %d] Buf Read: %d\n%s\n", g_wcore[sockid], rd, buf);
		...
		nevents = mtcp_epoll_wait(mctx, ep, events, MAX_EVENTS, -1);
				...
				for (i = 0; i < nevents; i++) {
					...
					if (events[i].events & MTCP_EPOLLERR) {
						...
						ret = HandleReadEvent(ctx, events[i].data.sockid,&svars[events[i].data.sockid]);
						g_total_msgs[0]++;
						if (ret == 0) {
							/* connection closed by remote host */
							CloseConnection(ctx, events[i].data.sockid, &svars[events[i].data.sockid]);
						} else if (ret < 0) {
						...
						struct server_vars *sv = &svars[events[i].data.sockid];
