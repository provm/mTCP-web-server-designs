...
PrintStats()
{
	...
	for (i = 0; i < core_limit; i++) {
		...
		/*fprintf(stderr, "[promod-cpu %d] Connects: %7lu Read: %4lu KB "
                        "Avg-resp-time: %4lu us Max-resp-time: %6lu us\n",
                        i, st->connects,
                        st->reads / 1024,
                        avg_resp_time, st->max_resp_time);*/
	...
	fprintf(stderr, "[Promod-All] Connects: %7lu Read: %4lu KB "
			"Avg-Resp-Time: %4lu us Max-Resp-Time: %6lu us\n",
			total.connects, 
			total.reads / (1024*1024),
			total_resp_time / core_limit, total.max_resp_time);
}
/*----------------------------------------------------------------------------*/
int done_time = FALSE;
...
RunWgetMain(void *arg)
{
	...
	while (ctx->started < ctx->target) {
		if (CreateConnection(ctx) < 0) {
			printf("[RunMain: Error Creating Connection]\n");
		}
	}

	fprintf(stderr, "[Core %d] %d Connections Started\n", core, n); 

	while (!done_time) {
		//gettimeofday(&cur_tv, NULL);
		//if (core == 0 && cur_tv.tv_sec > prev_tv.tv_sec+10) {
			//PrintStats();
		//	prev_tv = cur_tv;
		//}
...
main(int argc, char **argv)
{
	...
	strncpy(url, "/1kb.txt", MAX_URL_LEN);
