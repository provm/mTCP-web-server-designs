...
FlushEpollEvents(mtcp_manager_t mtcp, uint32_t cur_ts)
{
	...
	struct mtcp_epoll *ep_master;

	struct event_queue *usrq;
	...
	if(mtcp->w_dsmap == NULL){
		...
		ep_master=mtcp->ep_master;
		pthread_mutex_lock(&ep->epoll_lock);
		pthread_mutex_lock(&ep_master->epoll_lock);
	}
	PROMOD_TS1(PRO_EPOLL, "EPOLL")
	
	usrq = ep_master->usr_queue;
	...
	if (ep_master->waiting && (ep_master->usr_queue->num_events > 0 ||
				ep_master->usr_shadow_queue->num_events > 0)) {
		STAT_COUNT(mtcp->runstat.rounds_epoll);
		mtcp->ts_last_event = cur_ts;
		ep_master->stat.wakes++;
		pthread_cond_signal(&ep_master->epoll_cond);
	}
	if(mtcp->w_dsmap == NULL){
		...
		pthread_mutex_unlock(&ep->epoll_lock);
		pthread_mutex_unlock(&ep_master->epoll_lock);
	}
}
/*----------------------------------------------------------------------------*/
static inline void 
...
/*----------------------------------------------------------------------------*/
static void 
RunMainLoop(struct mtcp_thread_context *ctx, struct mtcp_thread_context *ctx_master, int w_id)
{
	mtcp_manager_t mtcp = ctx->mtcp_manager;
	int i;
	int recv_cnt;
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {
	    ...
	    for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++){
			//for(int cpu_n=0; cpu_n<8; cpu_n++){
			//   if(mtcp->cpu_bitset & 1<<cpu_n == 0 ) continue;
			...
			for (i = 0; i < recv_cnt; i++) {
					...
					if (pktbuf != NULL){
						printf("[core.c RunMainLoop] Core: %d | Packet Received\n", mtcp->ctx->cpu);
		...
		while(1){
			w_stream = StreamDequeue(mtcp->w_streamQ);
			if(w_stream == NULL) break;

			switch(w_stream->state){
				case TCP_ST_ESTABLISHED:
					Handle_TCP_ST_ESTABLISHED(mtcp, w_stream->cur_ts, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq, w_stream->payload, w_stream->payloadlen, w_stream->window);
					break;
				case TCP_ST_CLOSE_WAIT:
					Handle_TCP_ST_CLOSE_WAIT(mtcp, w_stream->cur_ts, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq, w_stream->payloadlen, w_stream->window);
					break;

				case TCP_ST_LAST_ACK:
					Handle_TCP_ST_LAST_ACK(mtcp, w_stream->cur_ts, w_stream->iph, w_stream->ip_len, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq, w_stream->payloadlen, w_stream->window);
					break;

				case TCP_ST_FIN_WAIT_1:
					Handle_TCP_ST_FIN_WAIT_1(mtcp, w_stream->cur_ts, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq, w_stream->payload, w_stream->payloadlen, w_stream->window);
					break;

				case TCP_ST_FIN_WAIT_2:
					Handle_TCP_ST_FIN_WAIT_2(mtcp, w_stream->cur_ts, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq, w_stream->payload, w_stream->payloadlen, w_stream->window);
					break;

				case TCP_ST_CLOSING:
					Handle_TCP_ST_CLOSING(mtcp, w_stream->cur_ts, w_stream, w_stream->tcph,
							w_stream->seq, w_stream->ack_seq,  w_stream->payloadlen, w_stream->window);
					break;
				...
				case TCP_ST_TIME_WAIT:
					...
					if (w_stream->on_timewait_list) {
						RemoveFromTimewaitList(mtcp, w_stream);
						AddtoTimewaitList(mtcp, w_stream, w_stream->cur_ts);
					}
					AddtoControlList(mtcp, w_stream, w_stream->cur_ts);
					break;
			}
		}
...
mtcp_ds_t AllocateDSMap(){
    mtcp_ds_t temp;
    //int i;

    temp = (mtcp_ds_t)calloc(1, sizeof(struct mtcp_ds));
    temp->smap_ds = (socket_map_t*)calloc(CONFIG.max_concurrency, sizeof(struct socket_map*));
    /*for (i = 0; i < CONFIG.max_concurrency; i++) {
    		temp->smap[i]=(socket_map_t)malloc(sizeof(struct socket_map));
    		temp->smap_ds[i].id = i;
    		temp->smap_ds[i].socktype = MTCP_SOCK_UNUSED;
    		memset(&temp->smap_ds[i].saddr, 0, sizeof(struct sockaddr_in));
    		temp->smap_ds[i].stream = NULL;
    }*/

    temp->type_ds = MTCP_IO;
    return temp;
}
...
// worker ctx, mctx | master ctx...
static mtcp_manager_t 
InitializeMTCPManager(struct mtcp_thread_context* ctx, mctx_t mctx, struct mtcp_thread_context* ctx_shared, int type_ds, int w_no)
{
	mtcp_manager_t mtcp, mtcp_master;
	char log_name[MAX_FILE_NAME];
	int i;
	struct timespec cur_s;
	...
	if (!mtcp) {
        ...
        g_mtcp[ctx->cpu] = mtcp;
        if(type_ds == MTCP_IO){
            mtcp_master = g_mtcp[ctx_shared->cpu];

            mtcp->w_dsmap = (mtcp_ds_t)AllocateDSMap();
            mtcp_master->m_dsmap[w_no]=mtcp->w_dsmap; 
        }
        else if(type_ds == MTCP_MASTER){
            for(i=0; i<MAX_SOCKET_WORKER;i++)
		g_wcpu[i] = g_wcpu[i] | (1<<20);
	    mtcp->w_dsmap = NULL;
	}

        mtcp->w_streamQ = CreateStreamQueue(15000);
        pthread_mutex_init(&mtcp->w_stream_lock, NULL);
	
        clock_gettime(CLOCK_REALTIME, &cur_s);	
	
	for(i=0;i<2;i++){
		mtcp->p_clock[i]=0;
		mtcp->p_nlock[i]=0;
		mtcp->s_clock[i]=cur_s.tv_sec;
	}
...
#define	IS_HUGEPAGE 0
	...
	if(type_ds == MTCP_MASTER) mtcp->ep = NULL;
	else{
		printf("[core.c InitializeMTCPManager] Creating Epoll Worker %d\n", ctx->cpu);
		mtcp_epoll_worker_create(mctx, MAX_EVENTS);
	}
...
MTCPRunThread(void *arg)
{
	...
	int cpu = mctx->cpu;
	int cpu_master = mctx->cpu_master;
	int w_id = mctx->w_id;
	int working;
	...
	if(w_id!=-1)
		mtcp = ctx->mtcp_manager = InitializeMTCPManager(ctx, mctx, g_pctx[cpu_master], MTCP_IO, w_id);
	else
		mtcp = ctx->mtcp_manager = InitializeMTCPManager(ctx, NULL, NULL, MTCP_MASTER, -1);
	...
	/* start the main loop */
	if(cpu_master != -1)
		RunMainLoop(ctx, g_pctx[cpu_master], w_id);
	else
		RunMainLoop(ctx, NULL, -1);
...
/*----------------------------------------------------------------------------*/
mctx_t 
mtcp_create_context(int cpu, int cpu_master, int type_ds, int w_id)
{
	mctx_t mctx;
	int ret;
...
{
	TRACE_CONFIG("Configuration Started\n");
	if (!conf)
		return -1;
