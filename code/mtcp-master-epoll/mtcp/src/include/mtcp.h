...
#define MAX_CPUS 16
#define MAX_SOCKET_WORKER 20000
...
{
	int cpu;
	int cpu_master;	// -1 if it is the master
	int w_id;
};
/*----------------------------------------------------------------------------*/
struct mtcp_sender
...
{
        int type_ds;
        socket_map_t *smap_ds;
};

typedef struct mtcp_ds* mtcp_ds_t;
...
struct mtcp_manager
{
    ...
    mtcp_ds_t m_dsmap[8];
    mtcp_ds_t w_dsmap;
    stream_queue_t w_streamQ;
    pthread_mutex_t w_stream_lock;
	...
	struct mtcp_epoll *ep_master; // Only for MTCP_IO
...
extern int g_wcpu[MAX_SOCKET_WORKER];
