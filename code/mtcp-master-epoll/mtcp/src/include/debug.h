...
#ifdef PROMOD
#define TRACE_PROMOD(f, m...) TRACE_FUNC("PROMOD", f, ##m)
#else
#define TRACE_PROMOD(f, m...)   (void)0
#endif

#define PRO_EPOLL 0
#define PRO_SBUFF 1

#define PROMOD_BEGIN	clock_t t1,t2; 			\
					long cPassed=0;		\
					struct timespec cur_s;

#define PROMOD_TS0	t1=clock();

#define PROMOD_TS1(LCK_ID, PSTR)																			\
					t2=clock();		 																	\
					cPassed=(t2-t1);																		\
					mtcp->p_clock[LCK_ID]+=cPassed;															\
					mtcp->p_nlock[LCK_ID]++;																\
					clock_gettime(CLOCK_REALTIME, &cur_s);														\
					if(cur_s.tv_sec>=mtcp->s_clock[LCK_ID]+10){												\
						TRACE_PROMOD("%s %ld %ld %ld\n", PSTR, cur_s.tv_sec, mtcp->p_clock[LCK_ID], mtcp->p_nlock[LCK_ID]);	\
						mtcp->s_clock[LCK_ID]=cur_s.tv_sec;	\
						mtcp->p_clock[LCK_ID]=0;	\
						mtcp->p_nlock[LCK_ID]=0;	\
					}
