...
/*----------------------------------------------------------------------------*/
int 
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen, int *core_id)
{
	mtcp_manager_t mtcp;
	struct tcp_listener *listener;
	...
	if (!accepted) {
		printf("[NEVER HAPPEN] Empty accept queue!\n");
	}

	if(*core_id == 0) w_id = accepted->cpu_id;
	else w_id = *core_id;
	...
	if(w_id!=0)
		mtcp->m_dsmap[w_id]->smap_ds[socket->id]=socket;
	
	g_mtcp[w_id]->flow_cnt++;
	g_wcpu[socket->id]=w_id;
...
CloseStreamSocket(mctx_t mctx, int sockid)
{
	...
	if(mtcp->w_dsmap == NULL)
		...
		cur_stream = mtcp->w_dsmap->smap_ds[sockid]->stream;
...
mtcp_close(mctx_t mctx, int sockid)
{
	...
	if(mtcp->w_dsmap == NULL)
		socket_w = mtcp->smap[sockid];
	else
		socket_w = *(mtcp->w_dsmap->smap_ds[sockid]);
	...
	if (socket_w.socktype == MTCP_SOCK_UNUSED) {
		TRACE_API("Invalid socket id: %d\n", sockid);
		errno = EBADF;
		return -1;
	}
...
mtcp_recv(mctx_t mctx, int sockid, char *buf, size_t len, int flags)
{
	...
	if(mtcp->w_dsmap == NULL)
		...
		socket = mtcp->w_dsmap->smap_ds[sockid];
...
mtcp_write(mctx_t mctx, int sockid, const char *buf, size_t len)
{
	...
	printf("[api.c mtcp_write] Core %d | Data Write | Socket: %d | Sending Data: %s\n", mtcp->ctx->cpu, sockid, buf);
	...
	if (!(socket->opts & MTCP_NONBLOCK)) {
		while (sndvar->snd_wnd <= 0) {
			printf("[api.c mtcp_write] Waiting for available sending window...\n");
			if (!cur_stream || cur_stream->state != TCP_ST_ESTABLISHED) {
