...
FilterSYNPacket(mtcp_manager_t mtcp, uint32_t ip, uint16_t port)
{
	...
	listener = (struct tcp_listener *)ListenerHTSearch(mtcp_master->listeners, &port);
		...
		uint32_t cur_ts, uint8_t *payload, uint32_t seq, int payloadlen)
{
	...
	if (!rcvvar->rcvbuf) {
		...
		if (!rcvvar->rcvbuf) {
			fprintf(stderr, "tcp_in.c Stream %d: Failed to allocate receive buffer.\n", 
					cur_stream->id);
			cur_stream->state = TCP_ST_CLOSED;
	...
	if (tcph->rst) {
		if (tcph->ack) {
			...
			if (cur_stream->socket) {
				fprintf(stderr, "[tcp_in.c Handle ST_SYN_SENT]\n");
				RaiseErrorEvent(mtcp, cur_stream);
			} else {
				DestroyTCPStream(mtcp, cur_stream);
			}
		...
		tcp_stream* cur_stream, struct tcphdr* tcph, uint32_t ack_seq) 
{
	...
	if (tcph->ack) {
		...
		mtcp_master = g_mtcp[0];
		printf("[tcp_in.c Handle_TCP_ST_SYN_RCVD] Core: %d | Adding Stream to Listner[%d] acceptQ\n", mtcp->ctx->cpu, 0);
		listener = (struct tcp_listener *)ListenerHTSearch(mtcp_master->listeners, &tcph->dest);
		...
		if (ret < 0) {
			printf("Stream %d: Failed to enqueue to "
					"the listen backlog!\n", cur_stream->id);
		...
		if (listener->socket && (listener->socket->epoll & MTCP_EPOLLIN)) {
			printf("[tcp_in.c Handle_SYN_RCVD] Core: %d | Adding Listen Event to MTCP_QUEUE\n", mtcp->ctx->cpu);
			AddEpollEvent(mtcp,mtcp->ep, 
		...
		uint8_t *payload, int payloadlen, uint16_t window) 
{
	...
	if (payloadlen > 0) {
		printf("[tcp_in.c Handle_TCP_ST_ESTABLISHED] Core: %d | Processing TCP Payload\n", mtcp->ctx->cpu);
...
/*----------------------------------------------------------------------------*/
void
Handle_TCP_ST_LAST_ACK (mtcp_manager_t mtcp, uint32_t cur_ts, struct iphdr *iph,
...
ProcessTCPPacket(mtcp_manager_t mtcp, 
		 uint32_t cur_ts, const int ifidx, struct iphdr *iph, int ip_len)
{
	...
	if (!(cur_stream = StreamHTSearch(mtcp->tcp_flow_table, &s_stream))) {
		...
		if (!cur_stream)
			return TRUE;
		cur_stream->cpu_id = mtcp->ctx->cpu;
	}
	...
	mtcp_w = NULL;
	if(cur_stream->socket!=NULL){
		cpu_w = g_wcpu[cur_stream->socket->id];
		mtcp_w = g_mtcp[cpu_w];
	}

	cur_stream->cur_ts = cur_ts;
	cur_stream->tcph = tcph;
	cur_stream->seq = seq;
	cur_stream->ack_seq = ack_seq;
	cur_stream->payload = payload;
	cur_stream->payloadlen = payloadlen;
	cur_stream->window = window;
	cur_stream->iph = iph;
	cur_stream->ip_len = ip_len;

	switch (cur_stream->state) {
	case TCP_ST_LISTEN:
		printf("[tcp_in.c ProcessTCPPacket] Core: %d | First SYN Received\n", mtcp->ctx->cpu);
	...
	case TCP_ST_SYN_RCVD:
		...
		printf("[tcp_in.c ProcessTCPPacket] Core: %d | ACK Received\n", mtcp->ctx->cpu);
	...
	case TCP_ST_ESTABLISHED:
		if(mtcp_w != NULL){
			printf("[tcp_in.c ProcessTCPPacket] Core: %d | Read Data Stream Enqueued to core[%d]\n", mtcp->ctx->cpu, mtcp_w->ctx->cpu);
			pthread_mutex_lock(&mtcp_w->w_stream_lock);
			StreamEnqueue(mtcp_w->w_streamQ, cur_stream);
			pthread_mutex_unlock(&mtcp_w->w_stream_lock);
		}
		else
			Handle_TCP_ST_ESTABLISHED(mtcp, cur_ts, cur_stream, tcph,
