...
#ifdef IP_DEFRAG
struct rte_mbuf *
ip_reassemble(struct dpdk_private_context *dpc, struct rtectxt->cpuo_mbuf *m)
{
...
dpdk_load_module(void)
{
	...
	if (!CONFIG.multi_process || (CONFIG.multi_process && CONFIG.multi_process_is_master)) {
		...
		TRACE_CONFIG("LOAD 2 %d\n",num_devices_attached);	
		/* Initialise each port */
		for (portid = 0; portid < num_devices_attached; portid++) {
			/* init port */
			TRACE_CONFIG("Initializing port %u... ", (unsigned) portid);
			ret = rte_eth_dev_configure(portid, CONFIG.num_cores, CONFIG.num_cores, &port_conf);
			
			TRACE_CONFIG("Load 2.1\n");	
			if (ret < 0)
				TRACE_CONFIG("Cannot configure device: err=%d, port=%u\n",
					 ret, (unsigned) portid);
			
			/* init one RX queue per CPU */
			TRACE_CONFIG("LOAD 2.1.0\n");
#ifdef DEBUG
